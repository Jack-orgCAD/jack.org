$(document).ready(function() {
  ...
  if ($("[data-donate='complete-button']").length > 0) {
    ...
    function checkPayPalReturn() {
      console.log('Checking PayPal return');
      ...
      console.log('PayPal payer ID:', payPalPayerId);
      console.log('Transaction ID:', transactionId);

      if (payPalToken && payPalPayerId && transactionId) {
        // Get the stored form data for this transaction
        const storedDataStr = sessionStorage.getItem('paypal_' + transactionId);
        console.log('Stored PayPal data:', storedDataStr);
        ...
        try {
          ...
          console.error('Error parsing stored PayPal data:', e);
          return false;
        }
        ...
        // Method 1: Try by ID selector
        if (storedData.formSelector) {
          $form = $(storedData.formSelector);
          console.log('Trying form selector:', storedData.formSelector, 'Found:', $form.length > 0);
        }

        // Method 2: Try to find any form with donation elements
        if (!$form || !$form.length) {
          $form = $('form')
            .filter(function () {
            ...
            .first();
          console.log('Trying to find form with donation elements. Found:', $form.length > 0);
        }

        // Method 3: Try to find by stored HTML
        if (!$form || !$form.length) {
          console.log('Trying to find form by stored HTML');
          const $allForms = $('form');
          $allForms.each(function () {
            const currentForm = $(this);
            if (currentForm.find('[data-donate="complete-button"]').length > 0) {
              $form = currentForm;
        ...
        if (!$form || !$form.length) {
          console.error('Could not find any valid donation form');
          return false;
        }

        console.log('Found form:', $form.attr('id') || 'no-id');
        ...
        // Get JWT token and submit the donation
        getJWTToken()
          .then(function (response) {
            jwtToken = response;
            ...
            let donationAmount;
            const $amountInput = $form.find('[data-donate="amount"] input:checked');
            console.log('Amount input found:', $amountInput.length > 0);

            if (!$amountInput.length) {
              console.error('No amount input found in form');
            ...
            if (!selectedAmount) {
              console.error('Selected amount is empty or undefined');
              throw new Error('Invalid donation amount');
            }

            if (selectedAmount.trim() === 'Other') {
              const otherAmount = $form.find('[data-donate="other-amount"]').val();
              console.log('Other amount:', otherAmount);

              if (!otherAmount) {
                console.error('Other amount is empty or undefined');
                throw new Error('Invalid other amount');
              }

              donationAmount = otherAmount.trim().replace('$', '');
            } else {
              donationAmount = selectedAmount.trim().replace('$', '');
            }

            console.log('Final donation amount:', donationAmount);

            // Create PayPal data object with all required fields
            const paypalData = {
              payPalToken: payPalToken,
              payPalPayerId: payPalPayerId,
              payPalTotalAmount: parseFloat(donationAmount),
              payPalCurrency: 'CAD', // Using CAD as default currency
              paymentMethod: 1, // 1 indicates PayPal payment
            };

            return formatAndSubmitDonation($form, null, null, paypalData);
          })
          .then(function (response) {
            // Clear the stored PayPal data
          ...
          .catch(function (error) {
            ...
            // Show error message
            $form.find('#cc-error').text('There was an error processing your PayPal payment. Please try again or contact support.').show();
    ...
    function initiatePayPalPayment($form) {
      // Validate form
      if (!$form || !$form.length) {
        console.error('Invalid form provided to initiatePayPalPayment');
        return;
      }

      // Generate a unique form identifier
      const formId = $form.attr('id') || $form.attr('data-donate-form') || $form.find('[data-donate="complete-button"]').attr('id') || 'donation-form-' + Date.now();
      ...
      // Store both form selector and form data
      const storedData = {
        formSelector: formSelector,
        formData: formData,
        timestamp: Date.now(),
        formHtml: $form.prop('outerHTML'), // Store the form HTML as a backup
      };

      console.log('Storing PayPal data:', {
        transactionId,
        formSelector,
        formId,
        storedData,
      ...
      getJWTToken()
        ...
        .then(function (response) {
          console.log('PayPal init response:', response); // Debug log
          ...
          // Check for PayPal URL in response
          if (response && (response.PayPalUrl || response.payPalUrl)) {
            const paypalUrl = response.PayPalUrl || response.payPalUrl;
            const paypalToken = response.PayPalToken || response.payPalToken;

            // Store the PayPal token in sessionStorage for later use
            ...
            console.error('Invalid PayPal response:', response); // Debug log
    ...
    function createPayPalPayload($form, donationAmount, returnUrl, cancelUrl) {
      ...
      const donationType = (() => {
        if (isDedicatedDonation) {
          switch (frequency) {
            case 'one-time':
              return formData.inMemory ? 3 : 2; // In Memory or In Honour Donation
            case 'monthly':
              return formData.inMemory ? 12 : 7; // In Memory or In Honour Monthly
            case 'quarterly':
              return formData.inMemory ? 21 : 9; // In Memory or In Honour Quarterly
            case 'annual':
              return formData.inMemory ? 22 : 10; // In Memory or In Honour Annual
            default:
              return formData.inMemory ? 3 : 2; // Default to One-time
          }
        } else {
          switch (frequency) {
            case 'one-time':
              return 1; // General Donation
            case 'monthly':
              return 4; // General Monthly
            case 'quarterly':
              return 5; // General Quarterly
            case 'annual':
              return 6; // General Annual
            default:
              return 1; // Default to General Donation
          }
        }
      })();

      // Format phone number to match working payload format
      ...
      // Return the payload wrapped in an array to match the working payload structure
      return [
        {
          profile: {
          ...
          purchaseItems: [
            {
              ...
              sponsoredEntityType: SponsoredEntityType.Event,
    ...
    function formatAndSubmitDonation($form, moneris_dataKey, moneris_bin, paypalData) {
      ...
      const donationType = (() => {
        if (isDedicatedDonation) {
          switch (frequency) {
            case 'one-time':
              return inMemory ? 3 : 2; // In Memory or In Honour Donation
            case 'monthly':
              return inMemory ? 12 : 7; // In Memory or In Honour Monthly
            case 'quarterly':
              return inMemory ? 21 : 9; // In Memory or In Honour Quarterly
            case 'annual':
              return inMemory ? 22 : 10; // In Memory or In Honour Annual
            default:
              return inMemory ? 3 : 2; // Default to One-time
          }
        } else {
          switch (frequency) {
            case 'one-time':
      ...
      const jsonData = {
        ...
        paymentDetails: {
          ...
          cvv: null,
          isVisaCheckOutAllowed: false,
          payPalCurrency: paypalData ? paypalData.payPalCurrency : null,
          payPalPayerId: paypalData ? paypalData.payPalPayerId : null,
          payPalToken: paypalData ? paypalData.payPalToken : null,
          payPalTotalAmount: paypalData ? paypalData.payPalTotalAmount : 0,
          paymentMethod: paypalData ? paypalData.paymentMethod : 0,
          reCaptchaError: null,
          transactionAttribute: null,
        },
        purchaseItems: [
      ...
      if (isAdminFee) {
        ...
        adminFeeAmount = Math.min(adminFeeAmount, 5.0);
