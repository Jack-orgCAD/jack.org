$(document).ready(function () {
    ...
    const CONFIG = {
      ...
      organizationId: 196,
      defaultSubEvent: 'fallappeal2026',
    ...
    const formHelpers = {
      getFormData($form) {
        ...
        const fields = {
          ...
          isCharitableOrg: $form.find('[data-donate="charitable-org"] input[type=checkbox]').is(':checked'),
    ...
    const ui = {
      ...
      updateEcardDesigns(tributeType) {
        ...
        const cardMappings = {
          honour: {
            english: [ '2331', '2332', '4659', '4660', '4766', '4767' ],
            french: ['3326', '4770', '4771'],
          },
          memory: {
            english: ['2333', '4664', '4665', '4768', '4769'],
            french: ['3328', '4772', '4773'],
          },
        };
    ...
    const paymentProcessors = {
      async processCreditCard($form) {
        ...
        try {
          ...
          return paymentProcessors.handleDonationResponse(response, formData.frequency, formData.donationAmount, donationType);
        } catch (error) {
      ...
      handleDonationResponse(response, frequency, donationAmount = 0, donationType = 0) {
        console.log('🔵 handleDonationResponse called', { response, frequency, donationType });
        ...
        if (parsedResponse.Success === true) {
          ...
          dataLayer.push({
            ...
            ecommerce: {
                ...
                currency: "CAD",
                affiliation: "raisin",
                fundraising_campaign_name: "Jack.org",
                fundraising_campaign_id: "4412",
                form_id: Object.keys(CONFIG.utmSourceMapping)[0],
                form_name: "NEW Donate - Jack.org",
                form_type: "donation",
                items: [
                    {
                        item_id: `0_${donationAmount}`,
                        item_name: `$${donationAmount}`,
                        item_brand: "Jack.org",
                        item_category: "donation",
                        item_category2: frequency,
                        item_category3: "direct",          
                        item_variant: state.isFrench ? 'fr' : 'en',
                        item_list_name: "suggested-giving-amounts",                 
                        price: donationAmount,
                        quantity: 1,
                        item_id_source: "synthetic",
    ...
    const monerisHandler = {
      ...
      async handleSuccess(respData) {
        ...
        try {
          ...
          const response = await api.submitDonation(donationData);
          const parsedResponse = paymentProcessors.handleDonationResponse(response, formData.frequency, formData.donationAmount, donationType);
